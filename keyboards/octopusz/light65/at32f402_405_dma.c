...
{
  uint32_t temp = 0;
  dmax_channely->ctrl_bit.chen = DMA_CONFIRM_FALSE;
  ...
  {
    /* dma1 channel */
    DMA1_LIB->clr |= (uint32_t)(0x0F << ((((temp & 0xFF) - 0x08) / 0x14) * 4));
  }
  else if((temp & 0x6FF) < 0x688)
  {
    /* dma2 channel */
    DMA2_LIB->clr |= (uint32_t)(0x0F << ((((temp & 0xFF) - 0x08) / 0x14) * 4));
  }
}
...
void dma_flag_clear(uint32_t dmax_flag)
{
  ...
  {
    DMA2_LIB->clr = (uint32_t)(dmax_flag & 0x0FFFFFFF);
  }
  else
  {
    DMA1_LIB->clr = dmax_flag;
  }
}
...
void dma_default_para_init(dma_init_type *dma_init_struct)
{
  ...
  dma_init_struct->peripheral_inc_enable = DMA_CONFIRM_FALSE;
  dma_init_struct->memory_inc_enable = DMA_CONFIRM_FALSE;
  dma_init_struct->peripheral_data_width = DMA_PERIPHERAL_DATA_WIDTH_BYTE;
  dma_init_struct->memory_data_width = DMA_MEMORY_DATA_WIDTH_BYTE;
  dma_init_struct->loop_mode_enable = DMA_CONFIRM_FALSE;
...
/**
  * @brief  dmamux init.
  * @param  dma_x: pointer to a dma_type structure, can be DMA1_LIB or DMA2_LIB.
...
{
  dma_x->muxsel_bit.tblsel = DMA_CONFIRM_TRUE;
...
{
  dmamux_sync_init_struct->sync_enable = DMA_CONFIRM_FALSE;
  dmamux_sync_init_struct->sync_event_enable = DMA_CONFIRM_FALSE;
...
{
  dmamux_gen_init_struct->gen_enable         = DMA_CONFIRM_FALSE;
...
void dmamux_sync_interrupt_enable(dmamux_channel_type *dmamux_channelx, dma_confirm_state new_state)
{
  if(new_state != DMA_CONFIRM_FALSE)
  {
    dmamux_channelx->muxctrl_bit.syncovien = DMA_CONFIRM_TRUE;
  }
  else
  {
    dmamux_channelx->muxctrl_bit.syncovien = DMA_CONFIRM_FALSE;
  }
}
...
void dmamux_generator_interrupt_enable(dmamux_generator_type *dmamux_gen_x, dma_confirm_state new_state)
{
  if(new_state != DMA_CONFIRM_FALSE)
  {
    dmamux_gen_x->gctrl_bit.trgovien = DMA_CONFIRM_TRUE;
  }
  else
  {
    dmamux_gen_x->gctrl_bit.trgovien = DMA_CONFIRM_FALSE;
  }
}
