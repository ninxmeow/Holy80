...
void crm_reset(void)
{
  ...
  crm_auto_step_mode_enable(CRM_CONFIRM_TRUE);

  /* set hicken bit */
  CRM_LIB->ctrl_bit.hicken = CRM_CONFIRM_TRUE;

  /* wait hick stable */
  while(CRM_LIB->ctrl_bit.hickstbl != SET);

  /* hick used as system clock */
  CRM_LIB->cfg_bit.sclksel = CRM_SCLK_HICK;

  /* wait sclk switch status */
  while(CRM_LIB->cfg_bit.sclksts != CRM_SCLK_HICK);

  /* reset cfg register, include sclk switch, ahbdiv, apb1div, apb2div, adcdiv, clkout bits */
  CRM_LIB->cfg = (0x40000000U);

  /* reset hexten, hextbyps, cfden and pllen bits */
  CRM_LIB->ctrl &= ~(0x010D0000U);

  /* reset pllms pllns pllfr pllrcs bits */
  CRM_LIB->pllcfg = 0x000007C1U;

  /* reset clkout_sel, clkoutdiv, pllclk_to_adc, hick_to_usb */
  CRM_LIB->misc1 &= 0x00005000U;
  CRM_LIB->misc1 |= 0x000F0000U;

  /* disable all interrupts enable and clear pending bits  */
  CRM_LIB->clkint = 0x009F0000U;

  /* disable auto step mode */
  crm_auto_step_mode_enable(CRM_CONFIRM_FALSE);
}

/**
  * @brief  enable or disable crm low speed external crystal bypass
  * @param  new_state (CRM_CONFIRM_TRUE or CRM_CONFIRM_FALSE)
  * @retval none
  */
void crm_lext_bypass(crm_confirm_state new_state)
{
  CRM_LIB->bpdc_bit.lextbyps = new_state;
}
...
/**
  ...
  * @retval none
  */
void crm_hext_bypass(crm_confirm_state new_state)
{
  CRM_LIB->ctrl_bit.hextbyps = new_state;
}
...
flag_status crm_interrupt_flag_get(uint32_t flag)
{
  ...
  switch(flag)
  {
    case CRM_LICK_READY_INT_FLAG:
      if(CRM_LIB->clkint_bit.lickstblf && CRM_LIB->clkint_bit.lickstblien)
      {
        status = SET;
      }
      break;
    case CRM_LEXT_READY_INT_FLAG:
      if(CRM_LIB->clkint_bit.lextstblf && CRM_LIB->clkint_bit.lextstblien)
      {
        status = SET;
      }
      break;
    case CRM_HICK_READY_INT_FLAG:
      if(CRM_LIB->clkint_bit.hickstblf && CRM_LIB->clkint_bit.hickstblien)
      {
        status = SET;
      }
      break;
    case CRM_HEXT_READY_INT_FLAG:
      if(CRM_LIB->clkint_bit.hextstblf && CRM_LIB->clkint_bit.hextstblien)
      {
        status = SET;
      }
      break;
    case CRM_PLL_READY_INT_FLAG:
      if(CRM_LIB->clkint_bit.pllstblf && CRM_LIB->clkint_bit.pllstblien)
      {
        status = SET;
      }
      break;
    case CRM_CLOCK_FAILURE_INT_FLAG:
      if(CRM_LIB->clkint_bit.cfdf && CRM_LIB->ctrl_bit.cfden)
      {
        status = SET;
      }
      break;
...
{
  CRM_LIB->ctrl_bit.hicktrim = trim_value;
}
...
void crm_hick_clock_calibration_set(uint8_t cali_value)
{
  ...
  CRM_LIB->misc1_bit.hickcal_key = 0x5A;

  /* write hick calibration value */
  CRM_LIB->ctrl_bit.hickcal = cali_value;

  /* disable write hick calibration */
  CRM_LIB->misc1_bit.hickcal_key = 0x0;
}
...
/**
  ...
  * @retval none
  */
void crm_periph_clock_enable(crm_periph_clock_type value, crm_confirm_state new_state)
{
  /* enable periph clock */
  if(CRM_CONFIRM_TRUE == new_state)
  {
...
/**
  ...
  * @retval none
  */
void crm_periph_reset(crm_periph_reset_type value, crm_confirm_state new_state)
{
  /* enable periph reset */
  if(new_state == CRM_CONFIRM_TRUE)
  {
    CRM_REG(value) |= (CRM_REG_BIT(value));
  }
...
/**
  ...
  * @retval none
  */
void crm_periph_lowpower_mode_enable(crm_periph_clock_lowpower_type value, crm_confirm_state new_state)
{
...
/**
  ...
  * @retval none
  */
void crm_clock_source_enable(crm_clock_source_type source, crm_confirm_state new_state)
{
  switch(source)
  {
    case CRM_CLOCK_SOURCE_HICK:
      CRM_LIB->ctrl_bit.hicken = new_state;
      break;
    case CRM_CLOCK_SOURCE_HEXT:
      CRM_LIB->ctrl_bit.hexten = new_state;
      break;
    case CRM_CLOCK_SOURCE_PLL:
      CRM_LIB->ctrl_bit.pllen = new_state;
      break;
    case CRM_CLOCK_SOURCE_LEXT:
      CRM_LIB->bpdc_bit.lexten = new_state;
      break;
    case CRM_CLOCK_SOURCE_LICK:
      CRM_LIB->ctrlsts_bit.licken = new_state;
      break;
    default: break;
  }
}
...
void crm_flag_clear(uint32_t flag)
{
  switch(flag)
  {
    ...
    case CRM_ALL_RESET_FLAG:
      CRM_LIB->ctrlsts_bit.rstfc = CRM_CONFIRM_TRUE;
      while(CRM_LIB->ctrlsts_bit.rstfc == CRM_CONFIRM_TRUE);
      break;
    case CRM_LICK_READY_INT_FLAG:
      CRM_LIB->clkint_bit.lickstblfc = CRM_CONFIRM_TRUE;
      break;
    case CRM_LEXT_READY_INT_FLAG:
      CRM_LIB->clkint_bit.lextstblfc = CRM_CONFIRM_TRUE;
      break;
    case CRM_HICK_READY_INT_FLAG:
      CRM_LIB->clkint_bit.hickstblfc = CRM_CONFIRM_TRUE;
      break;
    case CRM_HEXT_READY_INT_FLAG:
      CRM_LIB->clkint_bit.hextstblfc = CRM_CONFIRM_TRUE;
      break;
    case CRM_PLL_READY_INT_FLAG:
      CRM_LIB->clkint_bit.pllstblfc = CRM_CONFIRM_TRUE;
      break;
    case CRM_CLOCK_FAILURE_INT_FLAG:
      CRM_LIB->clkint_bit.cfdfc = CRM_CONFIRM_TRUE;
      break;
    default:
      break;
  }
...
{
  CRM_LIB->cfg_bit.ertcdiv = ((value & 0x1F0) >> 4);
  CRM_LIB->bpdc_bit.ertcsel = (value & 0xF);
}
...
/**
  ...
  * @retval none
  */
void crm_ertc_clock_enable(crm_confirm_state new_state)
{
  CRM_LIB->bpdc_bit.ertcen = new_state;
}
...
{
  CRM_LIB->misc2_bit.hext_to_sclk_div = value;
}
...
{
  CRM_LIB->misc2_bit.hick_to_sclk_div = value;
}
...
/**
  ...
  * @retval none
  */
void crm_clock_failure_detection_enable(crm_confirm_state new_state)
{
  CRM_LIB->ctrl_bit.cfden = new_state;
}
...
/**
  ...
  * @retval none
  */
void crm_battery_powered_domain_reset(crm_confirm_state new_state)
{
  CRM_LIB->bpdc_bit.bpdrst = new_state;
}
...
/**
  ...
  * @retval none
  */
void crm_auto_step_mode_enable(crm_confirm_state new_state)
{
  if(new_state == CRM_CONFIRM_TRUE)
    CRM_LIB->misc2_bit.auto_step_en = CRM_AUTO_STEP_MODE_ENABLE;
  else
    CRM_LIB->misc2_bit.auto_step_en = CRM_AUTO_STEP_MODE_DISABLE;
}
...
{
  CRM_LIB->cfg_bit.i2sf5clksel = value;
}
...
{
  __IO uint8_t temp_div = CRM_LIB->misc2_bit.hick_to_sclk_div;
  __IO uint8_t temp_sclk = CRM_LIB->misc1_bit.hick_to_sclk;
  ...
  CRM_LIB->misc1_bit.hick_to_sclk = CRM_CONFIRM_TRUE;
  CRM_LIB->misc1_bit.hickdiv = value;
  CRM_LIB->misc1_bit.hick_to_sclk = temp_sclk;
...
void crm_hick_sclk_frequency_select(crm_hick_sclk_frequency_type value)
{
  ...
  CRM_LIB->misc1_bit.hick_to_sclk = CRM_CONFIRM_TRUE;
  CRM_LIB->misc1_bit.hickdiv = CRM_HICK48_NODIV;
  CRM_LIB->misc1_bit.hick_to_sclk = value;
  crm_hick_sclk_div_set((crm_hick_sclk_div_type)temp_div);
}
...
void crm_usb_clock_source_select(crm_usb_clock_source_type value)
{
  ...
  CRM_LIB->misc2_bit.pllu_usb48_sel = value;
}
...
{
  CRM_LIB->otghs_bit.usbhs_phy12_sel = value;
}
                    ...
                    uint16_t pll_ms, crm_pll_fp_type pll_fp)
{
  ...
  CRM_LIB->pllcfg_bit.pllrcs = clock_source;

  /* config pll multiplication factor */
  CRM_LIB->pllcfg_bit.pllns = pll_ns;
  CRM_LIB->pllcfg_bit.pllms = pll_ms;
  CRM_LIB->pllcfg_bit.pllfp = pll_fp;
}
...
{
  CRM_LIB->pllcfg_bit.pllfu = pll_fu;
}

/**
  * @brief  config crm pllu output
  * @param  crm_confirm_state
  *         CRM_CONFIRM_TRUE or CRM_CONFIRM_FALSE
  * @retval none
  */
void crm_pllu_output_set(crm_confirm_state new_state)
{
  /* config pll clock source type output */
  CRM_LIB->pllcfg_bit.plluen = new_state;
}
...
{
  return (crm_sclk_type)CRM_LIB->cfg_bit.sclksts;
}
...
void crm_clocks_freq_get(crm_clocks_freq_type *clocks_struct)
{
  ...
  {
    case CRM_SCLK_HICK:
      if(((CRM_LIB->misc1_bit.hick_to_sclk) != RESET) && ((CRM_LIB->misc1_bit.hickdiv) != RESET))
        clocks_struct->sclk_freq = HICK_VALUE * 6;
      else
        clocks_struct->sclk_freq = HICK_VALUE;

      psc = CRM_LIB->misc2_bit.hick_to_sclk_div;
      clocks_struct->sclk_freq = clocks_struct->sclk_freq >> psc;
      break;
    case CRM_SCLK_HEXT:
      clocks_struct->sclk_freq = HEXT_VALUE;
      psc = CRM_LIB->misc2_bit.hext_to_sclk_div;
      clocks_struct->sclk_freq = clocks_struct->sclk_freq >> psc;
      break;
    case CRM_SCLK_PLL:
      /* get pll clock source */
      pll_clock_source = CRM_LIB->pllcfg_bit.pllrcs;

      /* get multiplication factor */
      pll_ns = CRM_LIB->pllcfg_bit.pllns;
      pll_ms = CRM_LIB->pllcfg_bit.pllms;
      pll_fr = pll_fp_table[CRM_LIB->pllcfg_bit.pllfp];
...
void crm_clock_out_set(crm_clkout_select_type clkout)
{
  if(clkout < 0x10)
  {
    CRM_LIB->cfg_bit.clkout_sel1 = (clkout & 0x3);
  }
  else
  {
    CRM_LIB->cfg_bit.clkout_sel1 = 0x1;
    CRM_LIB->misc1_bit.clkout_sel2 = (clkout & 0xF);
  }
}
...
{
  CRM_LIB->cfg_bit.clkoutdiv1 = div1;
  CRM_LIB->misc1_bit.clkoutdiv2 = div2;
}
...
/**
  ...
  * @retval none
  */
void crm_interrupt_enable(uint32_t crm_int, crm_confirm_state new_state)
{
  if(CRM_CONFIRM_TRUE == new_state)
    CRM_LIB->clkint |= crm_int;
  else
    CRM_LIB->clkint &= ~crm_int;
}
