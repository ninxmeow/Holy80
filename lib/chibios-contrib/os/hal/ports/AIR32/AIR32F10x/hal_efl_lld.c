...
/**
 * @file    hal_efl_lld.c
 * @brief   AIR32F1xx Embedded Flash subsystem low level driver source.
 *
 * @addtogroup HAL_EFL
 * @{
 */
...
#define AIR32_FLASH_LINE_SIZE               2U
#define AIR32_FLASH_LINE_MASK               (AIR32_FLASH_LINE_SIZE - 1U)
...
static const flash_descriptor_t efl_lld_descriptor = {
 ...
 .page_size         = AIR32_FLASH_LINE_SIZE,
 .sectors_count     = AIR32_FLASH_NUMBER_OF_BANKS *
                      AIR32_FLASH_SECTORS_PER_BANK,
 .sectors           = NULL,
 .sectors_size      = AIR32_FLASH_SECTOR_SIZE,
 .address           = (uint8_t *)FLASH_BASE,
 .size              = AIR32_FLASH_NUMBER_OF_BANKS *
                      AIR32_FLASH_SECTORS_PER_BANK *
                      AIR32_FLASH_SECTOR_SIZE
};
...
static inline void air32_flash_lock(EFlashDriver *eflp) {

  eflp->flash->CR |= FLASH_CR_LOCK;
}

static inline void air32_flash_unlock(EFlashDriver *eflp) {

  eflp->flash->KEYR |= FLASH_KEY1;
  eflp->flash->KEYR |= FLASH_KEY2;
}

static inline void air32_flash_enable_pgm(EFlashDriver *eflp) {

  eflp->flash->CR |= FLASH_CR_PG;
}

static inline void air32_flash_disable_pgm(EFlashDriver *eflp) {

  eflp->flash->CR &= ~FLASH_CR_PG;
}

static inline void air32_flash_clear_status(EFlashDriver *eflp) {

  eflp->flash->SR = 0x0000001FU;
}

static inline uint32_t air32_flash_is_busy(EFlashDriver *eflp) {

  return (eflp->flash->SR & FLASH_SR_BSY);
}

static inline void air32_flash_wait_busy(EFlashDriver *eflp) {

  /* Wait for busy bit clear.*/
  while (air32_flash_is_busy(eflp) != 0U) {
  }
}

static inline flash_error_t air32_flash_check_errors(EFlashDriver *eflp) {
...
flash_error_t efl_lld_program(void *instance, flash_offset_t offset,
  ...
  while (n > 0U) {
    ...
    union {
      uint16_t  hw[AIR32_FLASH_LINE_SIZE / sizeof (uint16_t)];
      uint8_t   b[AIR32_FLASH_LINE_SIZE / sizeof (uint8_t)];
    } line;
    ...
    address = (volatile uint16_t *)(efl_lld_descriptor.address +
                                    (offset & ~AIR32_FLASH_LINE_MASK));

    /* Copying data inside the prepared line.*/
    do {
      line.b[offset & AIR32_FLASH_LINE_MASK] = *pp;
      offset++;
      n--;
      pp++;
    }
    while ((n > 0U) & ((offset & AIR32_FLASH_LINE_MASK) != 0U));

    /* Programming line.*/
    address[0] = line.hw[0];
    air32_flash_wait_busy(devp);

    err = air32_flash_check_errors(devp);
...
flash_error_t efl_lld_query_erase(void *instance, uint32_t *wait_time) {
  ...
  if (devp->state == FLASH_ERASE) {
    ...
    if (air32_flash_is_busy(devp) == 0U) {
      ...
      /* Recommended time before polling again, this is a simplified
         implementation.*/
      if (wait_time != NULL) {
        *wait_time = (uint32_t)AIR32_FLASH_WAIT_TIME_MS;
      }
...
flash_error_t efl_lld_verify_erase(void *instance, flash_sector_t sector) {
  ...
  for (i = 0U; i < AIR32_FLASH_SECTOR_SIZE / sizeof(uint32_t); i++) {
