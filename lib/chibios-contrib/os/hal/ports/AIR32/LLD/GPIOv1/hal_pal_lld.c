 ...
 * @brief   AIR32 PAL low level driver code.
 *
 * @addtogroup PAL
 * @{
 */
...
/**
 * @brief   AIR32 I/O ports configuration.
 * @details Ports A-D(E, F, G) clocks enabled, AFIO clock enabled.
 *
 * @param[in] config    the AIR32 ports configuration
 *
 * @notapi
 */
void _pal_lld_init(const PALConfig *config) {
  ...
  // GPIOA->PUPDEN = config->PAData.pupden;
  // GPIOA->PUPDLOCK = config->PAData.pupdlock;
  GPIOB->ODR = config->PBData.odr;
  GPIOB->CRH = config->PBData.crh;
  GPIOB->CRL = config->PBData.crl;
  // GPIOB->PUPDEN = config->PBData.pupden;
  // GPIOB->PUPDLOCK = config->PBData.pupdlock;
  GPIOC->ODR = config->PCData.odr;
  GPIOC->CRH = config->PCData.crh;
  GPIOC->CRL = config->PCData.crl;
  // GPIOC->PUPDEN = config->PCData.pupden;
  // GPIOC->PUPDLOCK = config->PCData.pupdlock;
  GPIOD->ODR = config->PDData.odr;
  GPIOD->CRH = config->PDData.crh;
  GPIOD->CRL = config->PDData.crl;
  // GPIOD->PUPDEN = config->PDData.pupden;
  // GPIOD->PUPDLOCK = config->PDData.pupdlock;
}
...
void _pal_lld_setgroupmode(ioportid_t port,
  ...
  static const uint8_t cfgtab[] = {
    ...
    8,          /* Reserved.*/
    8,          /* Reserved.*/
    0xB,        /* PAL_MODE_AIR32_ALTERNATE_PUSHPULL, 50MHz.*/
    0xF,        /* PAL_MODE_AIR32_ALTERNATE_OPENDRAIN, 50MHz.*/
  };
...
void _pal_lld_enablepadevent(ioportid_t port,
  ...
  /* Multiple channel setting of the same channel not allowed, first disable
     it. This is done because on AIR32 the same channel cannot be mapped on
     multiple ports.*/
