 ...
 * @brief   AIR32 USB subsystem low level driver source.
 *
 * @addtogroup USB
 * @{
 */
...
static uint32_t usb_pm_alloc(USBDriver *usbp, size_t size) {
  ...
  osalDbgAssert(usbp->pmnext <= AIR32_USB_PMA_SIZE, "PMA overflow");
...
static size_t usb_packet_read_to_buffer(usbep_t ep, uint8_t *buf) {
  size_t i, n;
  air32_usb_descriptor_t *udp = USB_GET_DESCRIPTOR(ep);
  air32_usb_pma_t *pmap = USB_ADDR2PTR(udp->RXADDR0);
#if AIR32_USB_USE_ISOCHRONOUS
...
static void usb_packet_write_from_buffer(usbep_t ep,
                                         ...
                                         size_t n) {
  air32_usb_descriptor_t *udp = USB_GET_DESCRIPTOR(ep);
  air32_usb_pma_t *pmap = USB_ADDR2PTR(udp->TXADDR0);
  int i = (int)n;
  ...
  if (EPR_EP_TYPE_IS_ISO(epr) && (epr & EPR_DTOG_TX))
    udp->TXCOUNT1 = (air32_usb_pma_t)n;
  else
    udp->TXCOUNT0 = (air32_usb_pma_t)n;
#else
  udp->TXCOUNT0 = (air32_usb_pma_t)n;
#endif
  ...
  while (i >= 16) {
    ...
    w  = *(buf + 0);
    w |= *(buf + 1) << 8;
    *(pmap + 0) = (air32_usb_pma_t)w;
    w  = *(buf + 2);
    w |= *(buf + 3) << 8;
    *(pmap + 1) = (air32_usb_pma_t)w;
    w  = *(buf + 4);
    w |= *(buf + 5) << 8;
    *(pmap + 2) = (air32_usb_pma_t)w;
    w  = *(buf + 6);
    w |= *(buf + 7) << 8;
    *(pmap + 3) = (air32_usb_pma_t)w;
    w  = *(buf + 8);
    w |= *(buf + 9) << 8;
    *(pmap + 4) = (air32_usb_pma_t)w;
    w  = *(buf + 10);
    w |= *(buf + 11) << 8;
    *(pmap + 5) = (air32_usb_pma_t)w;
    w  = *(buf + 12);
    w |= *(buf + 13) << 8;
    *(pmap + 6) = (air32_usb_pma_t)w;
    w  = *(buf + 14);
    w |= *(buf + 15) << 8;
    *(pmap + 7) = (air32_usb_pma_t)w;
...
#if AIR32_USB_USE_USB1 || defined(__DOXYGEN__)
#if AIR32_USB1_HP_NUMBER != AIR32_USB1_LP_NUMBER
#if AIR32_USB_USE_ISOCHRONOUS
/**
 ...
 * @isr
 */
OSAL_IRQ_HANDLER(AIR32_USB1_HP_HANDLER) {
  uint32_t istr;
...
#endif /* AIR32_USB_USE_ISOCHRONOUS */
#endif /* AIR32_USB1_LP_NUMBER != AIR32_USB1_HP_NUMBER */
 ...
 * @isr
 */
OSAL_IRQ_HANDLER(AIR32_USB1_LP_HANDLER) {
  uint32_t istr;
  USBDriver *usbp = &USBD1;
  ...
  if ((istr & ISTR_SUSP) != 0U) {
    AIR32_USB->CNTR |= CNTR_FSUSP;
#if AIR32_USB_LOW_POWER_ON_SUSPEND
    AIR32_USB->CNTR |= CNTR_LP_MODE;
#endif
    _usb_suspend(usbp);
  }
...
void usb_lld_start(USBDriver *usbp) {
  ...
  if (usbp->state == USB_STOP) {
    ...
    if (&USBD1 == usbp) {
      ...
      osalDbgAssert((AIR32_USBCLK >= (48000000U - AIR32_USB_48MHZ_DELTA)) &&
                    (AIR32_USBCLK <= (48000000U + AIR32_USB_48MHZ_DELTA)),
      ...
      AIR32_USB->CNTR = CNTR_FRES;

      /* Enabling the USB IRQ vectors, this also gives enough time to allow
         the transceiver power up (1uS).*/
#if AIR32_USB1_HP_NUMBER != AIR32_USB1_LP_NUMBER
      nvicEnableVector(AIR32_USB1_HP_NUMBER, AIR32_USB_USB1_HP_IRQ_PRIORITY);
#endif
      nvicEnableVector(AIR32_USB1_LP_NUMBER, AIR32_USB_USB1_LP_IRQ_PRIORITY);
...
#if AIR32_USB1_HP_NUMBER != AIR32_USB1_LP_NUMBER
      nvicDisableVector(AIR32_USB1_HP_NUMBER);
#endif
      nvicDisableVector(AIR32_USB1_LP_NUMBER);

      AIR32_USB->CNTR = CNTR_PDWN | CNTR_FRES;
      rccDisableUSB();
    }
#endif
  }
...
void usb_lld_reset(USBDriver *usbp) {
  ...
  AIR32_USB->BTABLE = BTABLE_ADDR;
  AIR32_USB->ISTR   = 0U;
  AIR32_USB->DADDR  = DADDR_EF;
...
void usb_lld_set_address(USBDriver *usbp) {
  ...
  AIR32_USB->DADDR = (uint32_t)(usbp->address) | DADDR_EF;
}
...
void usb_lld_init_endpoint(USBDriver *usbp, usbep_t ep) {
  ...
  AIR32_USB->EPR[ep] = AIR32_USB->EPR[ep];
  AIR32_USB->EPR[ep] = epr | ep;
}
...
void usb_lld_disable_endpoints(USBDriver *usbp) {
  ...
  for (i = 1; i <= USB_ENDPOINTS_NUMBER; i++) {
    ...
    AIR32_USB->EPR[i] = AIR32_USB->EPR[i];
    AIR32_USB->EPR[i] = 0U;
  }
}
...
usbepstatus_t usb_lld_get_status_out(USBDriver *usbp, usbep_t ep) {
  ...
  (void)usbp;
  switch (AIR32_USB->EPR[ep] & EPR_STAT_RX_MASK) {
...
usbepstatus_t usb_lld_get_status_in(USBDriver *usbp, usbep_t ep) {
  ...
  (void)usbp;
  switch (AIR32_USB->EPR[ep] & EPR_STAT_TX_MASK) {
...
void usb_lld_read_setup(USBDriver *usbp, usbep_t ep, uint8_t *buf) {
  air32_usb_pma_t *pmap;
  air32_usb_descriptor_t *udp;
  uint32_t n;
...
void usb_lld_clear_out(USBDriver *usbp, usbep_t ep) {
  ...
  /* Makes sure to not put to NAK an endpoint that is already
     transferring.*/
  if ((AIR32_USB->EPR[ep] & EPR_STAT_RX_MASK) != EPR_STAT_RX_VALID) {
...
void usb_lld_clear_in(USBDriver *usbp, usbep_t ep) {
  ...
  /* Makes sure to not put to NAK an endpoint that is already
     transferring.*/
  if ((AIR32_USB->EPR[ep] & EPR_STAT_TX_MASK) != EPR_STAT_TX_VALID) {
