...
/**
 * @file    USBv1/air32_usb.h
 * @brief   AIR32 USB registers layout header.
 * @note    This file requires definitions from the ST AIR32 header files
 *          air32f10x.h.
 *
 * @addtogroup USB
 * @{
 ...
 */
#if AIR32_USB_ACCESS_SCHEME_2x16
typedef uint16_t air32_usb_pma_t;
#else
typedef uint32_t air32_usb_pma_t;
#endif
...
typedef struct {
  ...
  /*
   * @brief   DP pull up Register.
   */
  volatile uint32_t             DP_PUUP;
} air32_usb_t;
...
typedef struct {
   ...
   */
  volatile air32_usb_pma_t      TXADDR0;
  /**
   * @brief   TX counter register 0.
   */
  volatile air32_usb_pma_t      TXCOUNT0;
  /**
   * @brief   RX buffer offset register.
   */
  volatile air32_usb_pma_t      RXADDR0;
  /**
   * @brief   RX counter register 0.
   */
  volatile air32_usb_pma_t      RXCOUNT0;
} air32_usb_descriptor_t;
 ...
 */
  ...
  #define AIR32_USB_BASE        USB1_BASE
#elif defined(USB_BASE)
  #define AIR32_USB_BASE        USB_BASE
#else
  #define AIR32_USB_BASE        (APB1PERIPH_BASE + 0x5C00)
#endif
 ...
 */
  ...
  #define AIR32_USBRAM_BASE     USB1_PMAADDR
#elif defined(USB_PMAADDR)
  #define AIR32_USBRAM_BASE     USB_PMAADDR
#elif defined(USB_DRD_PMAADDR)
#define AIR32_USBRAM_BASE       USB_DRD_PMAADDR
#else
  #define AIR32_USBRAM_BASE     (APB1PERIPH_BASE + 0x6000)
#endif

/**
 * @brief Pointer to the USB registers block.
 */
#define AIR32_USB               ((air32_usb_t *)AIR32_USB_BASE)

/**
 * @brief   Pointer to the USB RAM.
 */
#define AIR32_USBRAM            ((air32_usb_pma_t *)AIR32_USBRAM_BASE)
...
#define EPR_CLEAR_CTR_RX(ep)                                                \
  AIR32_USB->EPR[ep] = (AIR32_USB->EPR[ep] & ~EPR_CTR_RX & ~EPR_TOGGLE_MASK)\
                       | EPR_CTR_TX

#define EPR_CLEAR_CTR_TX(ep)                                                \
  AIR32_USB->EPR[ep] = (AIR32_USB->EPR[ep] & ~EPR_CTR_TX & ~EPR_TOGGLE_MASK)\
                       | EPR_CTR_RX
...
#define USB_GET_DESCRIPTOR(ep)                                              \
  ((air32_usb_descriptor_t *)((uint32_t)AIR32_USBRAM_BASE +                 \
                              (uint32_t)AIR32_USB->BTABLE +                 \
                              (uint32_t)(ep) *                              \
                              sizeof(air32_usb_descriptor_t)))

/**
 * @brief   Converts from a PMA address to a physical address.
 */
#define USB_ADDR2PTR(addr)                                                  \
  ((air32_usb_pma_t *)((addr) *                                             \
                       (sizeof(air32_usb_pma_t) / 2) +                      \
                       AIR32_USBRAM_BASE))
